...
class ExternalContact
{
	 ...
	 * 最后更新：2022/06/23
	 * 企业同时授权了服务商的第三方应用与代开发应用，服务商可使用该接口将代开发应用获取到的external_userid转换为第三方应用的external_userid，从而进行关联映射。若代开发自建应用已升级（升级说明见 代开发应用安全性升级），则获取到的external_userid与第三方应用一致，故无须调用该接口。
	 *
	 * 请求方式：POST（HTTPS）
	 ...
	 */
	public function toServiceExternalUserid($external_userid)
	{
		$params = array();
		$params['external_userid'] = $external_userid;
		$rst = $this->_request->get($this->_url . 'to_service_external_userid', $params);
		return $this->_client->rst($rst);
	}
	 ...
	 */
	public function getNewExternalUserid($external_userid_list)
	{
		$params = array();
		$params['external_userid_list'] = $external_userid_list;
		$rst = $this->_request->get($this->_url . 'get_new_external_userid', $params);
		return $this->_client->rst($rst);
	}
}
